...
Game::Game()
    : mWindow(nullptr), mSpriteShader(nullptr), mIsRunning(true),
...
auto Game::get_texture(std::string const &filename)
  ...
  try {
    auto tex = std::make_shared<gl::texture>(filename);
    this->textures.emplace(filename, tex);
    return tex;
